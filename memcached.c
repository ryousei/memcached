 ...
 */
static void drive_machine(mctx_t mc, int ep, int listener);
static int new_socket(struct addrinfo *ai,mctx_t mc);
static int try_read_command(conn *c);
void SignalHandler(int sigmun);
//	struct conn *c;
...
//static pthread_t app_thread[12];
...
conn *conn_new(const int sfd, enum conn_states init_state,
        ...
        const int read_buffer_size, enum network_transport transport,mctx_t mc, conn *co, int ep) {
    //printf("conn:fd:%d\n",sfd);
    struct conn *c = &co[sfd];
    ...
    ev.data.sockid = sfd;
    mtcp_epoll_ctl(mc, ep, MTCP_EPOLL_CTL_ADD, sfd, &ev);
    // printf("end conn fd: %d, sfd:%d\n",c->sfd,sfd);
    return c;
}
...
static void conn_release_items(conn *c) {
    ...
    if (c->suffixleft != 0) {
        for (; c->suffixleft > 0; c->suffixleft--, c->suffixcurr++) {
            cache_free(suffix_cache, *(c->suffixcurr));
        }
    }
 ...
 * Outputs a protocol-specific "out of memory" error. For ASCII clients,
 i this is equivalent to out_string().
 */
        ...
        while(key_token->length != 0) {
            ...
            if (it) {
                ...
                if (return_cas)
                {
                    ...
                    suffix = cache_alloc(suffix_cache);
                    if (suffix == NULL) {
                        STATS_LOCK();
                        stats.malloc_fails++;
                        STATS_UNLOCK();
...
static enum try_read_result try_read_network(conn *c) {
    ...
    while (1) {
        ...
        res = mtcp_read(c->mctx,c->sfd, c->rbuf + c->rbytes, avail);
        //printf("buf: %s  end, avail: %d, res: %d \n",c->rbuf+c->rbytes,avail, res);
        if (res > 0) {
            //printf("in res>9 \n");
            //pthread_mutex_lock(&c->thread->stats.mutex);
            //c->thread->stats.bytes_read += res;
...
void do_accept_new_conns(const bool do_accept) {
    ...
    for (next = listen_conn; next; next = next->next) {
        if (do_accept) {
            ...
            if (mtcp_listen(next->mctx,next->sfd, settings.backlog) != 0) {
                perror("listen");
            }
        }
        else {
            update_event(next, 0);
            if (mtcp_listen(next->mctx,next->sfd, 0) != 0) {
                perror("listen");
            }
        }
    }
...
static enum transmit_result transmit(conn *c) {
    ...
    if (c->msgcurr < c->msgused) {
        ...
        //printf("transmit \n");
        res = mtcp_writev(c->mctx,c->sfd, m->msg_iov, m->msg_iovlen);
        //printf("transmit2 \n");
...
static void drive_machine(mctx_t mctx,int ep, int lis) {
    bool stop = false;
    int sfd;
    socklen_t addrlen;
    ...
    int listener = lis;
    struct mtcp_epoll_event ev;
    struct mtcp_epoll_event *events = (struct mtcp_epoll_event *) calloc(30000,sizeof(struct mtcp_epoll_event));  
    struct conn  *c = (struct conn *)calloc(10000,sizeof(struct conn));
#ifdef HAVE_ACCEPT4
    ...
    while (1) {
        //printf("in loop \n");
        nevents = mtcp_epoll_wait(mctx, ep, events, 30000, -1);
        //printf("nevent : %d\n",nevents);
        if(nevents <0){
            if(errno != EINTR)
...
#endif
#else
                sfd = mtcp_accept(mctx,events[i].data.sockid, (struct sockaddr *)&addr, &addrlen);
#endif
                //printf("sfd:%d\n",sfd);
                if(sfd<0)
                    break;
...
#else
                mtcp_setsock_nonblock(mctx,sfd);
#endif

                if (settings.maxconns_fast &&
                        stats.curr_conns + stats.reserved_fds >= settings.maxconns - 1) {
                    str = "ERROR Too many open connections\r\n";
                    res = mtcp_write(mctx,sfd, str, strlen(str));
                    mtcp_close(mctx,sfd);
                    STATS_LOCK();
                    ...
                    //                      DATA_BUFFER_SIZE, tcp_transport,c->mctx);
                    conn_new(sfd, conn_read, EV_READ | EV_PERSIST, DATA_BUFFER_SIZE, tcp_transport,mctx, c, ep);
                    //printf("af con fd : %d sfd : %d\n",c[sfd].sfd,sfd);
                }

                //stop = true
                break;
            }else if(events[i].events & MTCP_EPOLLIN || events[i].events & MTCP_EPOLLOUT){
                //printf("sw fd :%d, state: %u\n",events[i].data.sockid,c[events[i].data.sockid].state);     
                stop = false;
                while(!stop){
                    switch(c[events[i].data.sockid].state){
                        case conn_listening:
                            break;
                        case conn_waiting:
                            //printf("waiting mode \n");
                             ...
                             */
                            conn_set_state(&c[events[i].data.sockid], conn_read);
                            stop = true;
                            break;

                        case conn_read:
                            //printf("read mode \n");
                            res = IS_UDP(c[events[i].data.sockid].transport) ? try_read_udp(&c[events[i].data.sockid]) : try_read_network(&c[events[i].data.sockid]);
                            //printf("read mode come\n");
                            switch (res) {
                                case READ_NO_DATA_RECEIVED:
                                    //printf("nodata res:%d\n",res);
                                    conn_set_state(&c[events[i].data.sockid], conn_waiting);
                                    //	conn_set_state(&c[events[i].data.sockid], conn_closing);
                                    break;
                                case READ_DATA_RECEIVED:
                                    conn_set_state(&c[events[i].data.sockid], conn_parse_cmd);
                                    //try_read_command(&c[events[i].data.sockid]);
                                    //conn_set_state(&c[events[i].data.sockid], conn_waiting);
                                    break;
                                case READ_ERROR:
                                    //printf("data error res:%d\n",res);
                                    conn_set_state(&c[events[i].data.sockid], conn_closing);
                                    break;
                        ...
                        case conn_parse_cmd :
                            //printf("parse mode \n");
                            if (try_read_command(&c[events[i].data.sockid]) == 0) {
                                /* wee need more data! */
                                conn_set_state(&c[events[i].data.sockid], conn_waiting);
                            }
                        ...
                        case conn_new_cmd:
                            ...
                            //printf("nreqs:%d\n",nreqs);
                            if (nreqs >= 0) {
                                reset_cmd_handler(&c[events[i].data.sockid]);
                            } else {
                                conn_set_state(&c[events[i].data.sockid], conn_waiting);
                                //printf("close new cmd \n");
                                //pthread_mutex_lock(&c->thread->stats.mutex);
                                //c->thread->stats.conn_yields++;
                                // pthread_mutex_unlock(&c->thread->stats.mutex);
                                if (c[events[i].data.sockid].rbytes > 0) {
                        ...
                        case conn_nread:
                            //printf("nread mode \n");
                            if (c[events[i].data.sockid].rlbytes == 0) {
                                //printf("commpete \n");
                                complete_nread(&c[events[i].data.sockid]);
                                break;
                            }
                            //printf("nread mode 2 \n");
                            ...
                            if (c[events[i].data.sockid].rlbytes < 0) {
                                if (settings.verbose) {
                                    fprintf(stderr, "Invalid rlbytes to read: len %d\n", c[events[i].data.sockid].rlbytes);
                                }
                                conn_set_state(&c[events[i].data.sockid], conn_closing);
                                break;
                            }
                            ...
                            if (c[events[i].data.sockid].rbytes > 0) {
                                int tocopy = c[events[i].data.sockid].rbytes > c[events[i].data.sockid].rlbytes ? c[events[i].data.sockid].rlbytes : c[events[i].data.sockid].rbytes;
                                if (c[events[i].data.sockid].ritem != c[events[i].data.sockid].rcurr) {
                                    memmove(c[events[i].data.sockid].ritem, c[events[i].data.sockid].rcurr, tocopy);
                                }
                                c[events[i].data.sockid].ritem += tocopy;
                                c[events[i].data.sockid].rlbytes -= tocopy;
                                c[events[i].data.sockid].rcurr += tocopy;
                                c[events[i].data.sockid].rbytes -= tocopy;
                                if (c[events[i].data.sockid].rlbytes == 0) {
                                    break;
                                }
                            }
                            ...
                            res = mtcp_read(c[events[i].data.sockid].mctx,c[events[i].data.sockid].sfd, c[events[i].data.sockid].ritem, c[events[i].data.sockid].rlbytes);
                            printf("buf2: %s\n",c->ritem);
                            if (res > 0) {
                                //pthread_mutex_lock(&c->thread->stats.mutex);
                                //c->thread->stats.bytes_read += res;
                                //pthread_mutex_unlock(&c->thread->stats.mutex);
                                if (c[events[i].data.sockid].rcurr == c[events[i].data.sockid].ritem) {
                                    c[events[i].data.sockid].rcurr += res;
                                }
                                c[events[i].data.sockid].ritem += res;
                                c[events[i].data.sockid].rlbytes -= res;
                                break;
                            }
                        ...
                        case conn_swallow:
                            ...
                            if (c[events[i].data.sockid].sbytes == 0) {
                                conn_set_state(&c[events[i].data.sockid], conn_new_cmd);
                                break;
                            }

                            /* first check if we have leftovers in the conn_read buffer */
                            if (c[events[i].data.sockid].rbytes > 0) {
                                int tocopy = c[events[i].data.sockid].rbytes > c[events[i].data.sockid].sbytes ? c[events[i].data.sockid].sbytes : c[events[i].data.sockid].rbytes;
                                c[events[i].data.sockid].sbytes -= tocopy;
                                c[events[i].data.sockid].rcurr += tocopy;
                                c[events[i].data.sockid].rbytes -= tocopy;
                                break;
                            }
                            ...
                            res = mtcp_read(c[events[i].data.sockid].mctx,c[events[i].data.sockid].sfd, c[events[i].data.sockid].rbuf, c[events[i].data.sockid].rsize > c[events[i].data.sockid].sbytes ? c[events[i].data.sockid].sbytes : c[events[i].data.sockid].rsize);
                            if (res > 0) {
                                //pthread_mutex_lock(&c->thread->stats.mutex);
                                //c->thread->stats.bytes_read += res;
                                //pthread_mutex_unlock(&c->thread->stats.mutex);
                                c[events[i].data.sockid].sbytes -= res;
                                break;
                            }
                            if (res == 0) { /* end of stream */
                                conn_set_state(&c[events[i].data.sockid], conn_closing);
                            ...
                            /* otherwise we have a reval error, on which we close the connection */
                            if (settings.verbose > 0)
                                fprintf(stderr, "Failed to read, and not due to blocking\n");
                            conn_set_state(&c[events[i].data.sockid], conn_closing);
                            break;
                        ...
                        case conn_write:
                             ...
                             */
                            //printf("conn_write\n");
                            if (c[events[i].data.sockid].iovused == 0 || (IS_UDP(c[events[i].data.sockid].transport) && c[events[i].data.sockid].iovused == 1)) {
                                if (add_iov(&c[events[i].data.sockid], c[events[i].data.sockid].wcurr, c[events[i].data.sockid].wbytes) != 0) {
                                    if (settings.verbose > 0)
                        ...
                        case conn_mwrite:
                            //printf("conn_mwrite \n");
                            if (IS_UDP(c[events[i].data.sockid].transport) && c[events[i].data.sockid].msgcurr == 0 && build_udp_headers(&c[events[i].data.sockid]) != 0) {
                                if (settings.verbose > 0)
                            ...
                            switch (transmit(&c[events[i].data.sockid])) {
                                case TRANSMIT_COMPLETE:
                                    if (c[events[i].data.sockid].state == conn_mwrite) {
                                        conn_release_items(&c[events[i].data.sockid]);
                                        /* XXX:  I don't know why this wasn't the general case */
                                        if(c->protocol == binary_prot) {
                                            conn_set_state(&c[events[i].data.sockid], c[events[i].data.sockid].write_and_go);
                                        } else {
                                            conn_set_state(&c[events[i].data.sockid], conn_new_cmd);
                                        }
                                    } else if (c[events[i].data.sockid].state == conn_write) {
                                        if (c[events[i].data.sockid].write_and_free) {
                                            free(c[events[i].data.sockid].write_and_free);
                                            c[events[i].data.sockid].write_and_free = 0;
                                        }
                                        conn_set_state(&c[events[i].data.sockid], c[events[i].data.sockid].write_and_go);
                                    } else {
                                        if (settings.verbose > 0)
                                            fprintf(stderr, "Unexpected state %d\n", c[events[i].data.sockid].state);
                                        conn_set_state(&c[events[i].data.sockid], conn_closing);
                                    }
                                    break;
                        ...
                        case conn_closing:
                            if (IS_UDP(c[events[i].data.sockid].transport))
                                conn_cleanup(&c[events[i].data.sockid]);
                            else
                                conn_close(&c[events[i].data.sockid]);
                            stop = true;
                            break;
...
static int new_socket(struct addrinfo *ai,mctx_t mc) {
    ...
    if ((sfd = mtcp_socket(mc,ai->ai_family, ai->ai_socktype, ai->ai_protocol)) == -1) {
        return -1;
    }
...
static int server_socket(const char *interface,
        ...
        FILE *portnumber_file,mctx_t mc, int ep) {
    int sfd =0;
    ...
    for (next= ai; next; next= next->ai_next) {
        ...
        if (mtcp_bind(mc,sfd, next->ai_addr, next->ai_addrlen) == -1) {
            if (errno != EADDRINUSE) {
                perror("bind()");
                mtcp_close(mc,sfd);
                freeaddrinfo(ai);
            ...
            success++;
            if (!IS_UDP(transport) && mtcp_listen(mc,sfd, settings.backlog) == -1) {
                perror("listen()");
                mtcp_close(mc,sfd);
                freeaddrinfo(ai);
                return 1;
        ...
        if (IS_UDP(transport)) {
            ...
            for (c = 0; c < settings.num_threads_per_udp; c++) {
                ...
                dispatch_conn_new(per_thread_fd, conn_read,
                        EV_READ | EV_PERSIST,
                        UDP_READ_BUFFER_SIZE, transport,mc);
            }
            printf("UDP\n");
        } else {
            /*
...
static int server_sockets(int port, enum network_transport transport,
    ...
    if (settings.inter == NULL) {
        return server_socket(settings.inter, port, transport, portnumber_file,mc, ep);
    } else {
        ...
        for (char *p = strtok_r(list, ";,", &b);
            ...
            ret |= server_socket(p, the_port, transport, portnumber_file,mc, ep);
        }
        free(list);
        return ret;
    }
...
static int server_socket_unix(const char *path, int access_mask,mctx_t mc) {
    int sfd;
    struct linger ling = {0, 0};
...
RunServer(void *arg){
    int core = *(int *)arg;
    mctx_t tmp = malloc(sizeof(mctx_t));
    main_base = event_init(core,tmp);
    //set_mctx(main_base,0);
    ...
    if (settings.socketpath == NULL) {
        ...
        if (portnumber_filename != NULL) {
            snprintf(temp_portnumber_filename,sizeof(temp_portnumber_filenamae),"%s.lck", portnumber_filename);
        ...
        if (settings.port && server_sockets(settings.port, tcp_transport,
                    portnumber_file,mc)) {vperror("failed to listen on TCP port %d", settings.port);			exit(EX_OSERR);								               }
        errno = 0;
        if (portnumber_file) {
...
int main (int argc, char **argv) {
    ...
    //mctx_t tmp = get_mctx(NULL);
#if 0
    for (i = 0; i < 2; i++) {
        cores[i] = i;
        if (pthread_create(&app_thread[i], 
                    NULL, RunServer, (void *)&cores[i])    ) {
    ...
    //mctx_t tmp = malloc(sizeof(mctx_t));
    //main_base = event_init(0,tmp);
    //set_mctx(main_base,0);
    //mctx_t mc = get_mctx(main_base);
    int ep=0;
    int listener=0;
    mctx_t mc = mtcp_create_context(0);
    ep = mtcp_epoll_create(mc,30000);
    suffix_cache = cache_create("suffix", SUFFIX_SIZE, sizeof(char*),                                     NULL, NULL);
    ...
    if (settings.socketpath != NULL) {
        errno = 0;
        if (server_socket_unix(settings.socketpath,settings.access,mc)) {
    ...
    if (settings.socketpath == NULL) {
        ...
        errno = 0;
        listener = server_sockets(settings.port,tcp_transport,portnumber_file,mc,ep);
        //printf("listen : %d",listener);
        if (settings.port && 0) {
